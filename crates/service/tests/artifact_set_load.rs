...
//! downstream [`prove`](zkap_service::prove) trusts
...
fn artifact_set_load_trust_boundary() {
    ...
    for (field, mutate) in cases {
        ...
        mutate(&mut tampered);
        let err = ArtifactSet::load_unsigned(&tampered, &dir)
            .err()
    ...
    // surfaces as `artifacts.evm_verifier.sha256` mismatch. Using
    // Groth16Verifier.sol avoids risking a `CanonicalDeserialize`
    // failure on a flipped `pk.bin` byte.
    ...
    let err = ArtifactSet::load_unsigned(&canonical, &dir)
        .err()
        .expect("ArtifactSet::load_unsigned must reject a tampered Groth16Verifier.sol");
