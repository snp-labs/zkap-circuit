...
use gadget::utils::{is_less_than, single_multiplexer, slice_from_start, slice_grouped};
...
pub fn claim_extractor_v2<F: PrimeField>(
    ...
    let claim = slice_grouped::slice_efficient(payload, &pos.offset, &pos.claim_len, max_len)?;
    ...
    let result_value =
        slice_grouped::slice_efficient(payload, &absolute_value_offset, &pos.value_len, max_len)?;
...
mod tests {
    ...
    fn test_step_by_step_extraction() {
        ...
        let claim_extracted =
            slice_grouped::slice_efficient(&payload, &pos.offset, &pos.claim_len, 50).unwrap();
        println!(
        ...
        println!("\n=== Step 4: Extract value from claim ===");
        let result_value =
            slice_grouped::slice_efficient(&claim_extracted, &pos.value_idx, &pos.value_len, 50)
                .unwrap();
        println!(
