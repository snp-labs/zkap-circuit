...
//!   ([`fe_to_be32`], [`fe_from_be32_canonical`], [`field_to_hex`]).
//! - [`string`] — string-shaped inputs (decimal/hex JSON-style strings, padding,
//!   limb decomposition) — [`try_str_to_fields`], [`pad`], [`str_to_limbs`]; with
...
//! audit-driven hardening pass (P2 #14b). The companion split is the
//! sibling `ark-r1cs-helpers` crate for the R1CS gadget side; the
//! legacy `ark-utils` no longer exists.
