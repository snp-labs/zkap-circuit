//! Internal tests for `circuit::token::jwt_field`, extracted from the
//! inline `#[cfg(test)] mod tests` block in `src/token/jwt_field.rs`
//! (per-crate plan C10 / C-A7 U2).
//!
//! The tests cover both `jwt_nonce_hex_to_field` (hex domain, lives in
//! `jwt_field/nonce.rs`) and `jwt_exp_to_field` (decimal domain, lives in
//! `jwt_field/exp.rs`).  All symbols touched here are part of the crate's
//! public surface; this file is a literal AST relocation of the inline
//! tests with the only changes being the `super::*` → explicit
//! `circuit::token::jwt_field::{...}` import path adjustments required
