//! R1CS extension trait for 32-bit unsigned integers.
//!
//! Exports: [`UInt32Ext`].  Adds bitwise operations (`shr`, `not`, `bitand`)
//! to the arkworks `UInt32` type.  Requires the `r1cs` feature (default-on).

use core::iter;

use ark_ff::PrimeField;
use ark_r1cs_std::{prelude::Boolean, prelude::ToBitsGadget, uint32::UInt32};
use ark_relations::gr1cs::SynthesisError;

/// Bitwise helpers missing from the upstream [`UInt32`] gadget.
///
/// `from_bytes_be` is intentionally absent: arkworks 0.6 ships an inherent
/// `UInt::from_bytes_be` that shadows any same-named trait method, so an
/// extension method would be dead.
pub trait UInt32Ext<F: PrimeField>: Sized {
    /// Logical right shift by `by` positions. Returns
    /// `Err(SynthesisError::Unsatisfiable)` if `by >= 32`.
    fn shr(&self, by: usize) -> Result<Self, SynthesisError>;
    /// Bitwise NOT (one's complement). Propagates `SynthesisError` from
    /// `to_bits_le`, matching the pattern of sibling methods `shr` and
    /// `bitand`.
    fn not(&self) -> Result<Self, SynthesisError>;
    /// Bitwise AND with `other`. Both operands keep their existing R1CS
    /// allocations; only the resulting bits are newly allocated.
    fn bitand(&self, other: &Self) -> Result<Self, SynthesisError>;
}

impl<F: PrimeField> UInt32Ext<F> for UInt32<F> {
    fn shr(&self, by: usize) -> Result<Self, SynthesisError> {
        if by >= 32 {
            return Err(SynthesisError::Unsatisfiable);
        }

        let zeros = iter::repeat_n(Boolean::constant(false), by);
        let new_bits: Vec<_> = self
            .to_bits_le()?
            .into_iter()
            .skip(by)
            .chain(zeros)
            .collect();
        Ok(UInt32::from_bits_le(&new_bits))
    }

    fn not(&self) -> Result<Self, SynthesisError> {
        let new_bits: Vec<_> = self.to_bits_le()?.iter().map(|bit| !bit).collect();
        Ok(UInt32::from_bits_le(&new_bits))
    }

    fn bitand(&self, rhs: &Self) -> Result<Self, SynthesisError> {
        let new_bits: Vec<_> = self
            .to_bits_le()?
            .into_iter()
            .zip(rhs.to_bits_le()?)
            .map(|(a, b)| a & b)
            .collect();
        Ok(UInt32::from_bits_le(&new_bits))
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use ark_r1cs_std::{GR1CSVar, alloc::AllocVar, uint32::UInt32};
    use ark_relations::gr1cs::ConstraintSystem;

    type F = ark_bn254::Fr;

    #[test]
    fn test_uint32_shr_basic() {
        let _cs = ConstraintSystem::<F>::new_ref();
        let val = UInt32::<F>::constant(0xFF00u32);
        let shifted = val.shr(8).unwrap();
        assert_eq!(shifted.value().unwrap(), 0xFFu32);
    }

    #[test]
    fn test_uint32_shr_by_zero() {
        let _cs = ConstraintSystem::<F>::new_ref();
        let val = UInt32::<F>::constant(12345u32);
        let shifted = val.shr(0).unwrap();
        assert_eq!(shifted.value().unwrap(), 12345u32);
    }

    #[test]
    fn test_uint32_shr_by_31() {
        let _cs = ConstraintSystem::<F>::new_ref();
        let val = UInt32::<F>::constant(0x80000000u32);
        let shifted = val.shr(31).unwrap();
        assert_eq!(shifted.value().unwrap(), 1u32);
    }

    #[test]
    fn test_uint32_shr_by_32_returns_err() {
        let val = UInt32::<F>::constant(1u32);
        assert!(matches!(val.shr(32), Err(SynthesisError::Unsatisfiable)));
    }

    #[test]
    fn test_uint32_not_basic() {
        let _cs = ConstraintSystem::<F>::new_ref();
        let val = UInt32::<F>::constant(0u32);
        let notted = val.not().unwrap();
        assert_eq!(notted.value().unwrap(), 0xFFFFFFFFu32);

        let val2 = UInt32::<F>::constant(0xFFFFFFFFu32);
        let notted2 = val2.not().unwrap();
        assert_eq!(notted2.value().unwrap(), 0u32);
    }

    #[test]
    fn test_uint32_not_satisfiability() {
        // Allocate a known value, call not(), verify bits match bitwise-NOT
        // and that the resulting constraint system is satisfied.
        let cs = ConstraintSystem::<F>::new_ref();
        let input = 0xA5A5A5A5u32;
        let val = UInt32::<F>::new_witness(cs.clone(), || Ok(input)).unwrap();
        let result = val.not().unwrap();
        assert!(cs.is_satisfied().unwrap());
        assert_eq!(result.value().unwrap(), !input);
    }

    #[test]
    fn test_uint32_bitand_basic() {
        let cs = ConstraintSystem::<F>::new_ref();
        let a = UInt32::<F>::constant(0xFF00u32);
        let b = UInt32::<F>::constant(0x0FF0u32);
        let result = a.bitand(&b).unwrap();
        assert!(cs.is_satisfied().unwrap());
        assert_eq!(result.value().unwrap(), 0x0F00u32);
    }

}
